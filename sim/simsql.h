...
friend class SimSql;
public:
    SimSqlRes(MYSQL_RES* r=NULL, const int ec=0):res_(r), errcode_(ec){}
    ~SimSqlRes(){Free();}
...
public:
    ...
    SimSql():is_connected_(false), errcode_(0){}
    SimSql(const MysqlInfo& in):is_connected_(false), errcode_(0){
        Reset(in);
    }
    ~SimSql(){DisConnect();}
    bool Ping(){
    ...
    int TryNetQuery(const std::string& sql)
    {
        return TryQuery(sql);
        int ret = Query(sql);
        if (1 == errcode_ || (errcode_>= 1158 && errcode_ <=1161))
        {
    ...
    static inline const std::string Escape(const std::string& str) 
    {
        ...
        char to[tolen]={0};
        mysql_escape_string(to, str.c_str(), str.length());
        return to;
    }
private:
    bool is_connected_;
