...
public:
    ...
    int push_back(T t);
    int Add(T t){return push_back(t);}
...
public:
    ...
    ~SimPoolHandler(){Release();}
    T get(){return m_member;}
    T operator-> (){return m_member;}
    operator const bool (){return m_member != (T)0;}
    void Destroy(void (*free_func)(T t) = NULL);
    void swap(SimPoolHandler& h);
    SimPoolHandler* ptr(void){return this;}

private:
    void Release(); 
    //SimPoolHandler(const SimPoolHandler& _h){}
    SimPoolHandler& operator = (const SimPoolHandler& h){return *this;}
