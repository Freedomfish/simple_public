...
{
public:
    SimRedisReply(redisReply* r):reply_(r){}
    ~SimRedisReply(){Free();}
    redisReply* get(void) const {return reply_;}
    redisReply* operator->() const {return reply_;}
    SimRedisReply& operator=(redisReply* r){
        if (r != reply_){Free();reply_ = r;}
        return *this;
    }
...
{
public:
    SimRedis():m_errcode(0),m_ctxt(NULL){}
    SimRedis(const SimRedis& h):m_errcode(0),m_ctxt(NULL){*this = h;}
    SimRedis(const RedisInfo& _info):m_errcode(0),m_ctxt(NULL){Reset(_info);}
    SimRedis(const std::string& host, 
            const int port, 
            const struct timeval& time_out)
        :m_errcode(0), m_ctxt(NULL){Reset(host, port, time_out);}
    ~SimRedis(){ DisConnect();}
    SimRedis& operator=(const SimRedis& );

    bool is_connect()const {return !(m_ctxt == NULL);}
    int errcode()const {return m_errcode;}
    const std::string& errmsg()const {return m_errmsg;}
    const RedisInfo& info()const {return m_info;} 
    void swap(SimRedis& h);
    bool Ping(void);
    ...
    void Reset(const SimRedis& h){Reset(h.m_info);}

    int Connect(void);
    void DisConnect(void);
    int Update(void){DisConnect();return Connect();}
...
inline bool SimRedis::Ping(void)
{
...
inline bool SimRedis::Expire(const std::string& key, int expire_time)
{
