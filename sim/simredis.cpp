...
redisReply* SimRedis::Command(const char* format, ...)
{
    va_list argptr;
...
int SimRedis::AppendCommand(const char* format, ...)
{
    if (NULL == m_ctxt)
    {
        if (Connect()) 
...
int SimRedis::GetAllReply(size_t num, std::vector<redisReply* >& rp_vec)
{
    if (NULL == m_ctxt)
        return 1;
    if (m_ctxt->err)
...
redisReply* SimRedis::TryCommand(const char* format, ...)
{
    redisReply* r = NULL;
    va_list argptr;
    va_start(argptr,format);
...
redisReply* SimRedis::vTryCommand(const char* format, va_list argptr)
{
    redisReply* r = NULL;
    for (int i=0; i<2; i++)
    {
...
redisReply* SimRedis::TryCommandArgv(const std::vector<std::string>& argvec)
{
    redisReply* r=NULL;
    for (int i=0; i<2; i++)
    {
...
SimRedis::TryAppendArgvs(const std::vector<std::vector<std::string> >& argvs)
{
...
SimRedis& SimRedis::operator=(const SimRedis& h)
{
    if (this != &h)
    {
        DisConnect();
...
void SimRedis::Reset(const std::string& _host, 
		const int _port, 
...
redisReply* SimRedis::vCommand(const char* format, va_list argptr)
{
...
redisReply* SimRedis::CommandArgv(const std::vector<std::string>& argvec)
{
    if ((!m_ctxt || m_ctxt->err) && Update()) return NULL;
...
int SimRedis::AppendCommandArgv(const std::vector<std::string>& argvec)
{
...
std::vector<redisReply*> 
SimRedis::AppendArgvs(const std::vector<std::vector<std::string> >& argvs)
{
...
#if TEST
static struct timeval gtimeout = {1, 1000*1000};
int main()
{
    SimRedis rh("192.168.1.66", 6380, gtimeout);
    //RECONNECT_SERVER->start();
    redisReply *r = h.GetCommand("HGET %d %s", 370508642, "maxRecvSeq");
    if (r)
    {
        printf("seq:%s\n", r->str);
        freeReplyObject(r);
